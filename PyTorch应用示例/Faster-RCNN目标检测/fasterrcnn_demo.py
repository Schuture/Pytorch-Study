...
def vis_bbox(img, output, classes, max_vis=40, prob_thres=0.4): # 在原图中标出类别与bbox
...
class Compose(object): # 将transforms方法循环操作得到最终预处理完的数据
    def __init__(self, transforms):
        self.transforms = transforms

    def __call__(self, image, target):
        for t in self.transforms:
            image, target = t(image, target) # 这里的transform方法不仅作用在原图，也作用在bbox坐标、label
        return image, target
...
class RandomHorizontalFlip(object): # 随机水平翻转
    ...
    def __call__(self, image, target):
        if random.random() < self.prob:
            ...
            image = image.flip(-1) # 翻转原图
            bbox = target["boxes"]
            bbox[:, [0, 2]] = width - bbox[:, [2, 0]] # 把bbox的坐标也在原图中翻转
            target["boxes"] = bbox
        return image, target


class ToTensor(object): # 将原图、bbox坐标、label都转化为Tensor
    def __call__(self, image, target):
...
if __name__ == "__main__":
    ...
    # 收集batch data的函数，将Tensor转置
    def collate_fn(batch):
        return tuple(zip(*batch))

    train_loader = DataLoader(train_set, batch_size=batch_size, collate_fn=collate_fn)

    # step 2: model，加载预训练的模型，再微调最后几层
    ...
    params = [p for p in model.parameters() if p.requires_grad] # 只提取出最后几层需要梯度的参数来训练
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        for iter, (images, targets) in enumerate(train_loader):
            ...
            losses = sum(loss for loss in loss_dict.values()) # 模型本身最后一层就包含fastrcnn_loss，将输出加和即可
    ...
    preprocess = transforms.Compose([transforms.ToTensor(), ]) # 验证的时候，不考虑翻转
    ...
    for i in range(0, vis_num):
        ...
        # forward
        input_list = [img_chw]
        with torch.no_grad(): # 一定要记得no_grad()，以免验证时大量消耗资源
            tic = time.time()
